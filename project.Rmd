...
```{r load_data}
trainingdata <- read.csv('C:/Users/isak.rabin/Documents/Personal/Coursera/PML/project/pml-training.csv', na.strings=c('','NA','#DIV/0!'))
...
na.vars <- apply(trainingdata, 2, na.prop) > 0.5
...
na.excluded <- names(trainingdata) %in% na.columns
trainingdata <- trainingdata[!na.excluded]
dim(trainingdata)
```
...
nzv.vars <- nearZeroVar(trainingdata, saveMetrics=TRUE)
...
# Remove the NZV columns from the predictors
nzv.excluded <- names(trainingdata) %in% nzv.columns
trainingdata <- trainingdata[!nzv.excluded]
dim(trainingdata)
```

#####3. Remove Non-Predictors

Remove all variables that we juddges as non-predictors, i.e. variables that are judged as non consequential to the variable of interest.
...
```{r data_partition}
# Data Partitioning
inTrain <- createDataPartition(trainingdata$classe, p=.75, list=FALSE)
trainingdata.train <- trainingdata[ inTrain, ]; 
dim(trainingdata.train)
trainingdata.test  <- trainingdata[-inTrain, ]; 
dim(trainingdata.test)
```
...
```{r predicting}
# Apply transformations (i.e. cleaning of data) to trainingdata.test
trainingdata.test <- trainingdata.test[, names(trainingdata.train)]

# Prediction of trainingdata.test dataset
predict.rf <- predict(model.rf, newdata=trainingdata.test)
cm.rf <- confusionMatrix(predict.rf, trainingdata.test$classe)
...
##Conclusion
As can be seen from the confusion matrix above, this model is very accurate. 
I did experiment with PCA and other models, but did not get as good of accuracy.
Since test data achieve 99% accuracy, I expected nearly all of the submitted test cases to be correct. It turned out they were all correct.
...
`r n <- names(trainingdata.train); n[n != 'classe']`
...
# Prediction of trainingdata.test dataset
predict.dt <- predict(model.dt, newdata=trainingdata.test)
cm.dt <- confusionMatrix(predict.dt, trainingdata.test$classe)
...
plotcolors <- as.integer(unique(trainingdata$classe)) + 5
...
# Read Test Dataset
testingset <- read.csv('C:/Users/isak.rabin/Documents/Personal/Coursera/PML/project/pml-testing.csv', na.strings=c('','NA','#DIV/0!'))

# Select the same variables/columns as predictors
n <- names(trainingdata.train)
